...
server <- function(input, output, session) {
  ...
  observeEvent(input$show_prediction, {
    ...
    example_startq = gsub(":", " ", "2000:Q1")
    example_endq = gsub(":", " ", "2018:Q4")
    example_startyq = as.yearqtr(gsub(":", " ", "2000:Q1"))
    example_endyq = as.yearqtr(gsub(":", " ", "2018:Q4"))
    start_y = as.numeric(year(as.yearqtr(gsub(":", " ", "2000:Q1"))))
    start_q = as.numeric(quarter(as.yearqtr(gsub(":", " ", "2000:Q1"))))
    end_y = as.numeric(year(as.yearqtr(gsub(":", " ", "2018:Q4"))))
    end_q = as.numeric(quarter(as.yearqtr(gsub(":", " ", "2018:Q4"))))
    output$model3 <- renderPlot({
      ...
      spliced_GDP <- data_splice(RGDP_Data, "1947 Q1", "2023 Q4", "1965 Q4", 
                                 "2024 Q1", as.yearqtr(gsub(":", " ", "2000:Q1")), as.yearqtr(gsub(":", " ", "2018:Q4")), 3, 0)
      ...
      predictions <- check %>% 
        ...
        filter(Time > as.yearqtr(example_endq)) %>% 
        head(n = h) %>%
        mutate(new_growth_rate = c(-0.6193715, 2.518734))
      ...
      rmsfe_data <- cbind(time_data, fanplot_rmsfe(h)) 
...
print(model_3)
  ...
  observeEvent(input$show_prediction, {
    ...
    output$model4 <- renderPlot({
      ...
      predictions <- check %>% 
        mutate(Time = as.yearqtr(Dates)) %>%
        filter(Time > as.yearqtr(gsub(":", " ", "2018:Q4"))) %>% 
        head(n = h) %>%
        mutate(new_growth_rate = c(-0.5489326, 0.8056922))
  ...
  observeEvent(input$show_ADL, {   
    ...
    predictions <- check %>% 
      ...
      filter(Time > as.yearqtr(gsub(":", " ", "2018:Q4"))) %>% 
      head(n = h) %>%
      mutate(new_growth_rate = c(0.1234337, 2.343936))
