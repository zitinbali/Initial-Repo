...
function(input, output, session) {
  ...
  output$rolling_input <- renderUI({
    ...
    start = which(input$year[1]==RGDP_Data$DATE)+80
    end = length(RGDP_Data$DATE)-1
    
    selectInput("rolling_ADL",
                "Select End of Test Window: ",
                choices = RGDP_Data$DATE[start:end],
                selected = RGDP_Data$DATE[start]
    )
  })
  ...
  observeEvent(input$button1, {
    ...
    output$model1 <- renderPlotly({
      ...
      y_range = layer_scales(model_1)$y$range$range
  ...
  observeEvent(input$button6, {
    ...
    output$table6 <- DT::renderDataTable({
      ...
      predictions <- all_GDP_ts_df %>% 
        filter(Time > as.yearqtr(gsub(":", " ", input$year[2]))) %>% 
        head(n = h) %>%
        mutate(Date = as.character(Time), Predictions = pred_df$pred, RMSFE = pred_df$rmsfe) %>%
        select(Date, Predictions, RMSFE) %>% 
        datatable() %>% 
        formatRound(columns=c('Predictions', 'RMSFE'), digits=3)
    ...
    output$poor_outlook <- renderText({
      return(pred_df$outlook_message)
    })
    
    output$abnormal_high_indicators <- renderText({
      indicators <- if(is.null(pred_df$abnormal$high_dev_indicators)){
        "None"
      } else {
        paste(pred_df$abnormal$high_dev_indicators, collapse = ", ")
      }
      return(paste("Indicators with High Level Deviation:", indicators))
    })
    
    output$abnormal_med_indicators <- renderText({
      indicators <- if(is.null(pred_df$abnormal$medium_dev_indicators)){
        "None"
      } else {
        paste(pred_df$abnormal$medium_dev_indicators, collapse = ", ")
      }
      return(paste("Indicators with Medium Level Deviation:", indicators))
    })
    
    output$abnormal_message <- renderText({
      return(pred_df$abnormal$message)
    })
  ...
  observeEvent(input$button7, {
    ...
    window_start = as.yearqtr(check$Time[window_start_index+1])
  ...
  observeEvent(input$button8, {
    ...
    #X_df = baa_aaa_ts
    X_name = rename_variable(input$select_rolling_ADL)
    X_df = get(X_name)
