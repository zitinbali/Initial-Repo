...
#hstart_row_range <- c(279:926)
#hstart_col_range <- c(29:675)

# hstarts[hstart_row_range, hstart_col_range]

# Accounting for covid, stopping at Mar 2020 values

hstart_row_range <- c(279:879)
hstart_col_range <- c(29:628)

# hstarts[hstart_row_range, hstart_col_range]

post_prep_hstart <- prep_func(hstarts, hstart_row_range, hstart_col_range, 120)
...
hstarts_revised <- hstarts[hstart_row_range, hstart_col_range] %>%
    mutate_all(as.numeric)

colnames(hstarts_revised) <- sapply(seq(as.Date(as.yearmon("1970 M04", format = "%Y M%m")), by="month", length.out = ncol(hstarts_revised)), function(x) format(as.yearmon(x), format = "%Y M%m"))

rownames(hstarts_revised) <- sapply(seq(as.Date(as.yearmon("1970 M03", format = "%Y M%m")), by="month", length.out = nrow(hstarts_revised)), function(x) format(as.yearmon(x), format = "%Y M%m"))
...
ten_year_mark = (example_yq - 10 - as.yearqtr("1970 Q1", format = "%Y Q%q")) * 4
end_of_row_mark = (example_yq - as.yearqtr("1970 Q1", format = "%Y Q%q")) * 4 - 1 
...
# function to convert q to specific format 
QtoM = function(year_string){
  year = substr(year_string, start = 1, stop = 4)
  quarter = substr(year_string, start = 6, stop = 7)
  if (quarter == "Q1"){
    M = "M03"
  } else if (quarter == "Q2"){
    M = "M06"
...
example_mq = QtoM(example_q)
# example_mq_format is in the yearmon format 
example_mq_format = as.yearmon(example_mq, format = "%Y M%m") + 2/12


# Split dataframe into 10 years before target date and current value of growth for quarters more than 10 years ago
ten_year_mark_m = round((example_mq_format - 10 - 
                           as.yearmon("1970 M04", format = "%Y M%m")) * 12, 1)
end_of_row_mark_m = round((example_mq_format - 
                       as.yearmon("1970 M04", format = "%Y M%m")) * 12 - 1, 1)


# Most recent values for start of time to 10 years before target date
ancient_values_hstarts <-  perc_change_df_hstarts[1:ten_year_mark_m,][[`example_mq`]]

# All other values
recent_values_hstarts <- perc_change_df_hstarts[(ten_year_mark_m + 1):
                                  end_of_row_mark_m,][[`example_mq`]]

# Applying approximation of final growth numbers on recent values
forecast_growth_hstarts = recent_values_hstarts 
for (i in 1:length(recent_values_hstarts)){
  for (j in 1:i){
    forecast_growth_hstarts[i] = forecast_growth_hstarts[i] * (1 + (post_prep_hstart_delta[120 - j] / 100) )
  }
}
...
ADL1data <- ts.union(post_prep_gdp_df, TSpread_ts)
