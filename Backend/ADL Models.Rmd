...
AICselection_indv <- function(df, variable) {
  df <- data.frame(GDPGrowth_ts, variable)
...
ADL_predict <- function(Y_dataframe, X_dataframe, selectors, coefficients,
                        f_horizon){
  
  Y_string <- as.character(substitute(Y_dataframe))
  X_string <- as.character(substitute(X_dataframe))
  
  # the predictor might not be included at all 
...
ADL_BAA_AAA <- 
  dynlm(as.formula(AICselection_indv(baa_aaa_ts, "baa_aaa_ts")),
          start = c(start_y, start_q), 
          end = c(end_y, end_q))

BAA_AAA_selectors_AIC <- AICselection_indv(baa_aaa_ts, "baa_aaa_ts")
...
pred_2021q1_AIC_only <- ADL_predict(GDPGrowth_ts, baa_aaa_ts, BAA_AAA_selectors_AIC,
                                    ADL_BAA_AAA$coefficients, 4)
pred_2021q1_p_val <- ADL_predict(GDPGrowth_ts, baa_aaa_ts, BAA_AAA_selectors_p,
                                    ADL_BAA_AAA_f$coefficients, 4)
