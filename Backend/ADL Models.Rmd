...
generate_base_formula <- function(variable, lag, Y_name){
  ...
  formula <- paste(start_string, paste("L(", variable, ",", 1:lag, ")", sep = "", collapse = " + "))
  return(formula)
}
...
ADL_predict_all <- function(Y_dataframe, X_dataframe, f_horizon){
  ...
  if (f_horizon == 1){
    ...
    for (i in 2:f_horizon){
      ...
      print(i)
      #print(coeftest(model_AIC_local))
        
      # Formula recommended by p-value selector
      selectors_p_local <- pval_selection_indv(model_AIC_local, "gdp_ts")
      
      print(pval_selection_indv(model_AIC_local, "gdp_ts"))
      
      model_p_local <- dynlm(as.formula(selectors_p_local),
                         start = c(start_y, start_q), 
                         end = c(end_y, end_q))
      
      pred <- ADL_predict_1(gdp_ts, X_dataframe, Y_string, X_string,
