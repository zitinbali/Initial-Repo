
```{r import, warning = FALSE, results= "hide", echo = FALSE}

source("GDP Cleaning.R")
source("inputs.R")

```


# Basic GDP Prep 

```{r gdp prep, warning=FALSE}

spliced_GDP <- data_splice(RGDP_Data, "1947 Q1", "2023 Q4", "1965 Q4", 
                           "2024 Q1", example_startyq, example_endyq, 3, 0)

post_prep_gdp <- prep_func(spliced_GDP, 40)
post_prep_gdp_df <- post_prep_gdp$df
post_prep_gdp_delta = post_prep_gdp$delta

# revise GDP values

# note that the last input should be in a string format
sliced_perc_change <- data_splice(perc_change_df, "1947 Q2", "2023 Q4", 
                                  "1965 Q4", "2024 Q1", 
                                  example_startq, example_endq, 2, 1)
all_GDP_data <- revise_values(sliced_perc_change, post_prep_gdp_delta, 
                              example_startq, example_endq)

GDPGrowth_ts <- ts(all_GDP_data, 
                  start = c(start_y, start_q), 
                  end = c(end_y, end_q), 
                  frequency = 4)

# extract data from TS object for AR selection
#GDPGrowth_coredata = matrix(coredata(GDPGrowth_ts), ncol = 1)

# AR order selection for GDP growth rate
#models <- lapply(1:4, function(p) fitAR(GDPGrowth_coredata, p, 2)$model)
  
#aic <- aictab(cand.set = models, modnames = paste0('p=',1:4)) 

#best_lags <- as.integer(str_sub(aic[1,1], -1))

#GDPGrowth_selected <- models[[best_lags]]

# AIC picks AR(4) for GDP Growth 
```


# ADL Model - Splicing Function 

```{r adl splice function}

ADL_splice <- function(data, window_start, window_end){
  start_rownum = which(grepl(window_start, data$Date))
  end_rownum = which(grepl(window_end, data$Date))

  output <- data[start_rownum:end_rownum+1, ]
  
  return(output)
}

```


# ADL Model - BAA-AAA Spread 

```{r adl model baa-aaa}

baa_aaa <- read_excel("../Data/FRED BAA-AAA Data.xls", 
                      col_names = c("Date", "Spread")) %>% 
  mutate(Date = as.yearqtr(Date), 
         Spread = as.numeric(Spread))

baa_aaa <- ADL_splice(baa_aaa, example_startyq, example_endyq)
  
baa_aaa_ts <- ts(baa_aaa$Spread, 
                start = c(start_y, start_q), 
                end = c(end_y, end_q), 
                frequency = 4)

plot(merge(as.zoo(GDPGrowth_ts), as.zoo(baa_aaa_ts)), 
     plot.type = "single", 
     col = c("darkred", "steelblue"),
     lwd = 2,
     xlab = "Date",
     ylab = "",
     main = "BAA-AAA Spread & GDP Growth over Time")

legend("topright", 
       legend = c("GDP Growth", "BAA-AAA Spread"),
       col = c("darkred", "steelblue"),
       lwd = c(1, 1),
       cex = 0.5)

# THE NUMBER OF ADL MODELS IS CURRENTLY HARD CODED

# estimating the ADL(3,3) model of GDP growth

ADL_BAA_AAA33 <- 
  dynlm(GDPGrowth_ts ~ 
          L(GDPGrowth_ts) + L(GDPGrowth_ts, 2) + L(GDPGrowth_ts, 3) +
          L(baa_aaa_ts),
          start = c(start_y, start_q), 
          end = c(end_y, end_q))

BAA_AAA_coefficients <- ADL_BAA_AAA33$coefficients

coeftest(ADL_BAA_AAA33)

# extract data from TS object for AR selection
#BAA_AAA_coredata = matrix(coredata(baa_aaa_ts), ncol = 1)

# AR order selection for BAA-AAA spread 
#models <- lapply(1:4, function(p) fitAR(BAA_AAA_coredata, p, 2)$model)
  
#aic <- aictab(cand.set = models)

#best_lags <- as.integer(str_sub(aic[1,1], -1))

#BAA_AAA_selected <- models[[best_lags]]$call
# AIC picks AR(4) for BAA-AAA spread 

# THE ADL MODELS DO NOT HAVE A BUILT IN PRED FUNCTION

```


# ADL Model - Treasury Spread 

```{r adl model treasury}

tspread <- read_excel("../Data/FRED Treasury Spread.xls", col_names = c("Date", "Spread")) %>% 
  mutate(Date = as.yearqtr(Date), 
         Spread = as.numeric(Spread))

# NOTE: tspread is only from 1976 Q4 onward, so we can't accept forecast horizons earlier, at least not for this ADL model

tspread <- ADL_splice(tspread, example_startyq, example_endyq)

tspread_ts <- ts(tspread$Spread, 
                start = c(start_y, start_q), 
                end = c(end_y, end_q), 
                frequency = 4)

plot(merge(as.zoo(GDPGrowth_ts), as.zoo(tspread_ts)), 
     plot.type = "single", 
     col = c("darkred", "steelblue"),
     lwd = 2,
     xlab = "Date",
     ylab = "",
     main = "Treasury Spread & GDP Growth over Time")
legend("topright", 
       legend = c("GDP Growth", "Treasury Spread"),
       col = c("darkred", "steelblue"),
       lwd = c(1, 1),
       cex = 0.5)

# estimating the ADL(3,4) model of GDP growth

ADL_tspread34 <- 
  dynlm(GDPGrowth_ts ~ 
          L(GDPGrowth_ts) + L(GDPGrowth_ts, 2) + L(GDPGrowth_ts, 3) + 
          L(tspread_ts),
          start = c(start_y, start_q), 
          end = c(end_y, end_q))

tspread_coefficients <- ADL_tspread34$coefficients

coeftest(ADL_tspread34)

# extract data from TS object for AR selection
#tspread_coredata = matrix(coredata(tspread_ts), ncol = 1)

# AR order selection for Treasury spread
#models <- lapply(1:4, function(p) fitAR(tspread_coredata, p, 2)$model)
  
#aic <- aictab(cand.set = models)

#best_lags <- as.integer(str_sub(aic[1,1], -1))

#tspread_selected <- models[[best_lags]]$call
# AIC picks AR(3) for Treasury spread 

```


# ADL Model - FRED Hstarts

```{r adl model fred hstarts}

fred_hstarts <- read_excel("../Data/FRED Hstarts.xls", col_names = c("Date", "Spread")) %>% 
  mutate(Date = as.yearqtr(Date), 
         Spread = as.numeric(Spread))

fred_hstarts <- ADL_splice(fred_hstarts, example_startyq, example_endyq)

fred_hstarts_ts <- ts(fred_hstarts$Spread, 
                start = c(start_y, start_q), 
                end = c(end_y, end_q), 
                frequency = 4)

plot(merge(as.zoo(GDPGrowth_ts), as.zoo(fred_hstarts_ts)), 
     plot.type = "single", 
     col = c("darkred", "steelblue"),
     lwd = 2,
     xlab = "Date",
     ylab = "",
     main = "Housing Starts & GDP Growth over Time")
legend("topright", 
       legend = c("GDP Growth", "Housing Starts"),
       col = c("darkred", "steelblue"),
       lwd = c(1, 1),
       cex = 0.5)

# estimating the ADL(3,4) model of GDP growth

ADL_fred_hstarts31 <- 
  dynlm(GDPGrowth_ts ~ 
          L(GDPGrowth_ts) + L(GDPGrowth_ts, 2) + L(GDPGrowth_ts, 3) +
          L(fred_hstarts_ts),
          start = c(start_y, start_q), 
          end = c(end_y, end_q))

fred_hstarts_coefficients <- ADL_fred_hstarts31$coefficients

coeftest(ADL_fred_hstarts31)

# extract data from TS object for AR selection
#fred_hstarts_coredata = matrix(coredata(fred_hstarts_ts), ncol = 1)

# AR order selection for Hstarts
#models <- lapply(1:4, function(p) fitAR(fred_hstarts_coredata, p, 2)$model)
  
#aic <- aictab(cand.set = models)

#best_lags <- as.integer(str_sub(aic[1,1], -1))

#fred_hstarts_selected <- models[[best_lags]]$call
# AIC picks AR(4) for Hstarts

# HSTARTS VERY BAD.

```


# ADL Model - Consumer Sentiment

```{r adl model consent}

consent <- read_excel("../Data/FRED Consumer Sentiment.xls", col_names = c("Date", "Spread")) %>% 
  mutate(Date = as.yearqtr(Date), 
         Spread = as.numeric(Spread))

consent <- ADL_splice(consent, example_startyq, example_endyq)

consent_ts <- ts(consent$Spread, 
                start = c(start_y, start_q), 
                end = c(end_y, end_q), 
                frequency = 4)

plot(merge(as.zoo(GDPGrowth_ts), as.zoo(consent_ts)), 
     plot.type = "single", 
     col = c("darkred", "steelblue"),
     lwd = 2,
     xlab = "Date",
     ylab = "",
     main = "Consumer Sentiment & GDP Growth over Time")
legend("topright", 
       legend = c("GDP Growth", "Consumer Sentiment"),
       col = c("darkred", "steelblue"),
       lwd = c(1, 1),
       cex = 0.5)

# estimating the ADL(3,1) model of GDP growth

ADL_consent31 <- 
  dynlm(GDPGrowth_ts ~ 
          L(GDPGrowth_ts) + L(GDPGrowth_ts, 2) + L(GDPGrowth_ts, 3) +
          L(consent_ts),
          start = c(start_y, start_q), 
          end = c(end_y, end_q))

consent_coefficients <- ADL_consent31$coefficients

coeftest(ADL_consent31)

# extract data from TS object for AR selection
#consent_coredata = matrix(coredata(consent_ts), ncol = 1)

# AR order selection for consumer sentiment
#models <- lapply(1:4, function(p) fitAR(consent_coredata, p, 2)$model)
  
#aic <- aictab(cand.set = models)

#best_lags <- as.integer(str_sub(aic[1,1], -1))

#consent_selected <- models[[best_lags]]$call
# AIC picks AR(4) for consumer sentiment
```

# ADL Combined Model selection

```{r prep for AIC selection}
df <- data.frame(GDP_growth = GDPGrowth_ts, BAA_AAA_spread = baa_aaa_ts, treasury_spread = tspread_ts, housing_starts = fred_hstarts_ts, consumer_sentiment = consent_ts)

# List of variables
variables <- c("baa_aaa_ts", "tspread_ts", "fred_hstarts_ts", "consent_ts")

find_best_lag <- function(data, response, variable, max_lags) {
  best_aic <- Inf
  best_lag <- 0
  
  for (lag in 1:max_lags) {
    # Build the model formula
    formula <- as.formula(paste(response, "~", paste("L(", variable, ",", 1:lag, ")", sep = "", collapse = " + ")))
    
    # Fit the model
    model <- dynlm(formula, data = data)
    
    # Calculate AIC
    aic <- AIC(model)
    
    # Update best lag if AIC improves
    if (aic < best_aic) {
      best_aic <- aic
      best_lag <- lag
    }
  }
  return(best_lag)
}

generate_base_formula <- function(variable, lag){
  formula <- as.formula(paste("GDPGrowth_ts ~", paste("L(", variable, ",", 1:lag, ")", sep = "", collapse = " + ")))
  return(formula)
}

generate_formula_string <- function(base, variable, lag){
  formula_string <- paste("GDPGrowth_ts ~", 
                          paste0("L(", base, ",", 1:base_best_lag, ")", collapse = " + "), "+", 
                          paste0("L(", variable, ",", 1:lag, ")", collapse = " + "))
  formula <- as.formula(formula_string)
  return(formula)
}

```

```{r combined ADL without AR}
AICselection <- function(variable){
  base_best_lag = find_best_lag(df, "GDPGrowth_ts", variable, 4)
  base_formula = generate_base_formula(variable, base_best_lag)
  for(i in 1:3){
    var <- variables[i+1]
    best_aic <- AIC(dynlm(base_formula))
    best_lag = 0
    for(lag in 1:4){
      formula_string <- generate_formula_string(variable, var, lag)
      formula = update(base_formula, as.formula(formula_string))
      model = dynlm(formula, data = df)
      aic = AIC(model)
      if (aic < best_aic) {
        best_aic = aic
        best_lag = lag
      }
    }
    if(best_lag != 0){
        formula_string <- generate_formula_string(variable, var, best_lag)
        base_formula <- update(base_formula, as.formula(formula_string))
    }
  }
  return(base_formula)
}

AICselection("baa_aaa_ts")
```

```{r combined ADL with AR}
AICselection <- function(variable){
  base_best_lag = find_best_lag(df, "GDPGrowth_ts", variable, 4)
  base_formula = generate_base_formula(variable, base_best_lag)
  for(i in 1:4){
    var <- variables[i]
    best_aic <- AIC(dynlm(base_formula))
    best_lag = 0
    for(lag in 1:4){
      formula_string <- generate_formula_string(variable, var, lag)
      formula = update(base_formula, as.formula(formula_string))
      model = dynlm(formula, data = df)
      aic = AIC(model)
      if (aic < best_aic) {
        best_aic = aic
        best_lag = lag
      }
    }
    if(best_lag != 0){
        formula_string <- generate_formula_string(variable, var, best_lag)
        base_formula <- update(base_formula, as.formula(formula_string))
    }
  }
  return(base_formula)
}

AICselection("GDPGrowth_ts")
```