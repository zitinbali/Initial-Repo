...
```{r adl model combined}
# compare all variables with 4 lags of GDP growth rate 
#method 1
models <- list()

# Define the base formula
base_formula <- dynlm(GDPGrowth_ts ~ L(GDPGrowth_ts) + L(GDPGrowth_ts, 2) + L(GDPGrowth_ts, 3) + L(GDPGrowth_ts, 4))

# Loop to construct models with increasing lag terms for consent_ts
for (i in 0:4) {
  # Construct the formula dynamically
  formula_string <- paste("GDPGrowth_ts ~", paste0("L(GDPGrowth_ts,", 1:4, ")", collapse = " + "), "+", paste0("L(consent_ts,", 1:i, ")", collapse = " + "))
  formula <- as.formula(formula_string)
  
  # Fit the model using lm() and add it to the list of models
  models[[i + 1]] <- update(base_formula, formula)
}
###################
#method 2
base_formula =  GDPGrowth_selected

models <- list()

for(i in 0:4){
  base_formula <- update(base_formula, y ~ X + L(BAA_AAA_coredata,i+1))
  models[[i+1]] <- base_formula
}
