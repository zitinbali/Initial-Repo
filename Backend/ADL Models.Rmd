...
ADL_predict_all <- function(Y_dataframe, X_dataframe, f_horizon){
  ...
  if (f_horizon == 1){
    ...
    for (i in 2:f_horizon){
      ...
      # Formula recommended by AIC selector
      selectors_AIC_local <- AICselection_indv(GDPGrowth_ts, X_string)
      ...
      # Formula recommended by p-value selector
      selectors_p_local <- pval_selection_indv(model_AIC_local)
      
      print(pval_selection_indv(model_AIC_local))
      
      model_p_local <- dynlm(as.formula(selectors_p_local),
                         start = c(start_y, start_q), 
                         end = c(end_y, end_q))
      print("help 4")
      pred <- ADL_predict_1(GDPGrowth_ts, X_dataframe, Y_string, X_string,
