...
ADL_comb_predict <- function(Y_dataframe, X_combined_dataframe,
  ...
  if (covid_dummy_name %in% coef_row_name){
    ...
    # if it's 2020 Q1, take dummy coefficient * -1 
    if (end_yq == "2020 Q1"){
      temp = coef_df[covid_dummy_name, 1] * (-1)
      pred = temp + pred
    } 
    # if it's 2020 Q2, take dummy coefficient * 1 
    else if (end_yq == "2020 Q2"){
      temp = coef_df[covid_dummy_name, 1] * (1)
      pred = temp + pred
    }
  }
...
ADL_comb_predict_all(GDPGrowth_ts, X_comb_df, 2, example_endq)
...
t <- "GDPGrowth_ts ~ covid_dummy_ts + L(GDPGrowth_ts, 1) + L(baa_aaa_ts, 1) + L(fred_hstarts_ts, 1) +  L(GDPGrowth_ts, 2)"



model_AIC <- dynlm(as.formula(t),
        start = c(start_y, start_q), 
        end = c(end_y, end_q))

test_coefficients <- model_AIC$coefficients
test_coef_df <- as.data.frame(test_coefficients)
test_coef_row_name <- c(rownames(test_coef_df))
  
test_coef_df <- test_coef_df %>% 
mutate(lag_name = gsub("^.*?\\(([^,]+).*", "\\1", test_coef_row_name), 
       lag_num = str_sub(test_coef_row_name,-2,-2))

is.na(test_coef_df["covid_dummy_ts", test_coef_df$coefficients])
...
ADL_comb_predict_all(GDPGrowth_ts, X_comb_df, 2)
ADL_comb_predict_all(GDPGrowth_ts, X_comb_df, 3)
ADL_comb_predict_all(GDPGrowth_ts, X_comb_df, 4)


test_coef_row_name <- c("(Intercept)", "covid_dummy_ts", "L(GDPGrowth_ts, 1)", "L(baa_aaa_ts, 1)")
test_name <- "covid_dummy_ts"
test_name %in% test_coef_row_name
