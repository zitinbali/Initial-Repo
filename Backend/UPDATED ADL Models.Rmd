...
ADL_comb_predict <- function(Y_dataframe, X_combined_dataframe,
  ...
  if ("covid_dummy" %in% coef_row_name){
    # if it is null, ignore 
    if (is.null(coef_df["covid_dummy", coef_df$coefficients])){
      pred = pred
    } else {
      temp = coef_df["covid_dummy", coef_df$coefficients] * covid_dummy
      pred = temp + pred
    }
  }
...
ADL_comb_predict_all <- function(Y_dataframe, X_combined_dataframe, f_horizon){
  ...
  pred <- ADL_comb_predict(Y_dataframe, X_dataframe,
                        selectors_AIC, model_AIC$coefficients)
...
ADL_comb_predict_all(GDPGrowth_ts, X_comb_df, 1)

test_selectors_AIC <- "GDPGrowth_ts ~ covid_dummy + L(baa_aaa_ts,1) + L(fred_hstarts_ts,1) + L(GDPGrowth_ts,2) + L(fred_hstarts_ts,2)"

test_model <- dynlm(as.formula(test_selectors_AIC),
        start = c(start_y, start_q), 
        end = c(end_y, end_q))

test_model$coefficients

test_coef_df <- as.data.frame(test_model$coefficients)

t_coef_row_name <- c(rownames(test_coef_df))

test_coef_df <- test_coef_df %>% 
  mutate(lag_name = gsub("^.*?\\(([^,]+).*", "\\1", t_coef_row_name), 
         lag_num = str_sub(t_coef_row_name,-2,-2))

test_coef_df[1, 2]

# pred value is 0.451248

0.81286969 + (-0.36026118*1.3800000) + (0.02944165*1.54028) + (0.02163284*-3.54286) + 
  (0.33207651*0.50239160)
...
test_formula <- "GDPGrowth_ts ~ covid_dummy + L(baa_aaa_ts,1) + L(fred_hstarts_ts,1) + L(consent_ts,1) + L(GDPGrowth_ts,2) + L(fred_hstarts_ts,2) + L(consent_ts,2)"


test_model <- dynlm(as.formula(test_formula),
        start = c(start_y, start_q), 
        end = c(end_y, end_q))

coeftest(test_model)

# don't calculate the best model again for ADL combined prediction 

comb_AICselector(GDPGrowth_ts, X_comb_df, end_y, end_q, covid_dummy)
