...
AICselection_indv <- function(Y_dataframe, X_dataframe, Y_name) {
  df <- data.frame(Y_dataframe, X_dataframe)
  ...
  variable <- deparse(substitute(X_dataframe))
  for (lag in 1:4) {
...
ADL_predict_all <- function(Y_dataframe, X_dataframe, f_horizon){
  ...
  selectors_AIC <- AICselection_indv(Y_dataframe, X_dataframe, Y_string)
  ...
  # X pred 
  AR_output <- fitAR(X_df, 2, 1, covid_dummy)
  ...
  if (f_horizon == 1){
    ...
    for (i in 2:f_horizon){
      updated_end_yq <- example_endyq + i - 1
      ...
      # Formula recommended by AIC selector
      selectors_AIC_local <- AICselection_indv(gdp_ts, X_dataframe, "gdp_ts")
      ...
      AR_output_new <- fitAR(X_df, 2, i, covid_dummy)
